...
export async function buildDB() {
  ...
  await asyncForEach(buildDBIniFiles, async buildDBIniFile => {
    iniFilesContent[buildDBIniFile] = await readFile(path.join(buildSourceDBFolder, buildDBIniFile));
  });
  await asyncForEach(buildDBFolders, async buildDBFolder => {
    ...
    await asyncForEach(fileExtsInPath, async fileExt => {
      ...
      const destinationFilePath = path.join(buildDestinationDBFolder, destinationFileName);
      const filesNamesInPathWithExt = filesNamesInPath.filter( fileName => fileName.endsWith(fileExt) )
