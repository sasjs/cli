  ...
  const commands = [
    ...
    },
    {
      ...
      description: [
        ...
        `* ${chalk.cyanBright('sync')} - Synchronise the remote SAS file system with the local project folder according to the target 'syncDirectories' array.`,
        `[2spaces]command example: sasjs fs sync /Public/localFolder /Public/remoteFolder`,
        ``,
        `* ${chalk.cyanBright('compile')} - Compiles a SAS program with the contents of a local directory.`,
