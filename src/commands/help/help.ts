  ...
  const commands = [
    ...
    },
    {
      ...
      description: [
        ...
        `<path/to/log> - Location in which to store the log file. If not provided AND current execution path is sasjs project, it will be saved in sasjsresults else in the execution path`,
        `<path/to/output> - Location to store the output file. If not provided AND current location is sasjs project, a file named output.json will be saved in the sasjsresults folder else in execution path.`
      ]
    },
    {
      name: 'folder',
